    ...
    <nav className="container-navbar">
      <div className="navbar">
        <div className="nav-left animate__animated animate__bounceInLeft">
          ...
          <NavLink className="logo-text " to={"/portfolio-cisco-model"}>
            Cisco
          </NavLink>
        </div>
        <div className="nav-right animate__animated animate__bounceInRight  ">
          <NavLink
            style={(isActive) => ({ color: isActive ? "#fff" : "#b1ebe5" })}
            to={"/skills"}
          >
            {isTrue ? "Abilità" : "Skills"}
          </NavLink>
          <NavLink
            style={(isActive) => ({ color: isActive ? "#fff" : "#b1ebe5" })}
            to={"/projects"}
          >
            {isTrue ? "Progetti" : "Projects"}
          </NavLink>
          <NavLink
            style={(isActive) => ({ color: isActive ? "#fff" : "#b1ebe5" })}
            to={"/about"}
          >
            {isTrue ? "Chi Sono" : "About"}
          </NavLink>
          <NavLink
            style={(isActive) => ({ color: isActive ? "#fff" : "#b1ebe5" })}
            to={"/contact"}
          >
            {isTrue ? "Contatti" : "Contact"}
          </NavLink>
        </div>
      </div>
